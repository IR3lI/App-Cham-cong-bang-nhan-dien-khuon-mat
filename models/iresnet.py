"""
IResNet-50 Backbone (InsightFace / ArcFace compatible)
Kiến trúc phải khớp chính xác với cấu trúc dùng để train file .pth
"""

import torch
import torch.nn as nn
from torch.nn import BatchNorm1d, BatchNorm2d, Conv2d, Linear, Module
...
class IResNet(Module):
    """
    IResNet backbone dùng trong InsightFace / ArcFace.
    Input: ảnh khuôn mặt 3×112×112
    Output: embedding vector (512-d mặc định với IR-50)
    """
    ...
    def __init__(
        ...
        self.fc = Linear(512 * self.fc_scale, num_features)
...
def iresnet50(num_features=512, **kwargs):
    """Tạo IResNet-50 với 512-d embedding (mặc định ArcFace)."""
    model = IResNet(IBasicBlock, [3, 4, 14, 3], num_features=num_features, **kwargs)
    return model


def iresnet100(num_features=512, **kwargs):
    """Tạo IResNet-100 với 512-d embedding."""
    model = IResNet(IBasicBlock, [3, 13, 30, 3], num_features=num_features, **kwargs)
    return model
